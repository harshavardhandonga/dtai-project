import type { DemoClaim, ClaimAnalysisResult, AuditEvent, PipelineStatus } from "@/lib/types";
...
export function analyzeClaim(claim: DemoClaim, pipeline?: PipelineStatus): ClaimAnalysisResult {
  ...
  return {
    ...
    pipeline: pipeline ?? {
      mode: claim.aiMode,
      documentIntelligence: claim.aiMode === "LIVE" ? "live" : "cached",
      claimUnderstanding: claim.aiMode === "LIVE" ? "live" : "cached",
      message: claim.aiMode === "LIVE"
        ? "Live Azure document extraction and claim understanding completed."
        : "Cached deterministic demo evidence is active.",
    },
  };
}
