...
export const azureConfig = {
  ...
  // AZURE_OPENAI_BASE_URL is retained as a compatibility alias for the
  // original Config.docx. The PRD-standard name is AZURE_OPENAI_ENDPOINT.
  openaiEndpoint: norm(process.env.AZURE_OPENAI_ENDPOINT || process.env.AZURE_OPENAI_BASE_URL),
...
export function isDocumentIntelligenceConfigured(): boolean {
  return !!(azureConfig.diEndpoint && azureConfig.diKey);
}

export function isOpenAiConfigured(): boolean {
  return !!(azureConfig.openaiEndpoint && azureConfig.openaiKey && azureConfig.chatDeployment);
}

export function isEmbeddingConfigured(): boolean {
  return !!(isOpenAiConfigured() && azureConfig.embeddingDeployment);
}

export function isSearchConfigured(): boolean {
  return !!(azureConfig.searchEndpoint && azureConfig.searchKey && azureConfig.searchIndex);
}
