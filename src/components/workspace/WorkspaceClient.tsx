...
export function WorkspaceClient({ claimId }: { claimId: string }) {
  ...
  const askRAG = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/rag/query", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ question: ragQuestion, claimId }),
      });
  ...
  if (!analysis) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <p className="text-red-500">Failed to load claim analysis.</p>
      </div>
    );
  }
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <Card className="p-4">
            <div className="grid grid-cols-2 md:grid-cols-6 gap-4 text-sm">
              <div><p className="text-xs text-slate-500">Policy</p><p className="font-semibold text-slate-800">{claim.context.policySchedule.policyCode}</p></div>
